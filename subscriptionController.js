...
exports.updateUsedPoints = catchAsync(async (req, res) => {
  ...
  const subscription = await Subs.findByIdAndUpdate({ _id: id });
  // console.log(subscription);
  // console.log(subscription.subscription.usedPoints);
  let n = 1;
  const update = await Subs.findByIdAndUpdate(
    { _id: id },
    {
      $set: {
        'subscription.usedPoints': subscription.subscription.usedPoints + n,
      },
    }
  );
  // console.log('update' + update);
  const checkUsedPoints = await Subs.findById({ _id: id });
